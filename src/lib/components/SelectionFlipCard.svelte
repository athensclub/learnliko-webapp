...
<Flippable class={clazz} flip={flipped}>
	<button
		...
		slot="front"
		class="h-full w-full overflow-hidden bg-white bg-cover bg-center {$isMobile
			? 'rounded-[5vw]'
			: 'rounded-[2vw]'}"
	>
		<div
			class="flex h-full w-full flex-col items-center justify-between py-[6vw] backdrop-blur-[8px]"
		>
			<div
				style="box-shadow: 0px 2px 6px 0px rgba(0, 0, 0, 0.15);"
				class="mx-auto flex w-fit flex-row items-center rounded-full {$isMobile
			...
			{#if data.image}
				<img
					src={data.image}
					class="max-w-[80%] {$isMobile ? 'rounded-[2.5vw]' : 'rounded-[1vw]'}"
			...
			<div
				class="flex flex-row items-center font-bold {$isMobile ? 'text-[3.5vw]' : 'text-[1.3vw]'}"
			>
				<svg
					class={$isMobile ? 'mr-[2.5vw] w-[4.5vw]' : 'mr-[1vw] w-[2.3vw]'}
					viewBox="0 0 50 55"
					fill="none"
					xmlns="http://www.w3.org/2000/svg"
				>
	...
	>
		...
		<div class="relative flex h-full w-full flex-col items-center justify-center font-bold">
			<button
				on:click={() => (flipped = !flipped)}
			...
			{:else if correctAnswer === null}
				...
				>
					...
					<div class="flex flex-col gap-[7vw]">
						<button
							on:click={submit}
							disabled={selectedChoice === null}
							style="box-shadow: 0px 2px 8px 0px rgba(0, 0, 0, 0.25);"
							class="pointer-events-auto mx-auto w-[65%] rounded-full bg-white {$isMobile
						...
						<div
							class="mx-auto flex flex-row items-center font-bold {$isMobile
								? 'text-[3.5vw]'
								: 'text-[1.3vw]'}"
						>
							<svg
			...
			{:else}
				<div
					...
					class="pointer-events-none absolute left-0 top-0 flex h-full w-full flex-col items-center font-bold
                        ...
                        {$isMobile ? 'py-[6vw]' : 'py-[2vw]'}"
				>
					<div
						class="{$isMobile ? 'text-[6vw]' : 'text-[1.5vw]'} {correctAnswer === selectedChoice
							? 'bg-gradient-to-r from-[#6C80E8] to-[#9BA1FD] bg-clip-text text-transparent'
							: 'text-[#AAAAAA]'}"
					>
					...
					{#if data.image}
						<img
							src={data.image}
							class="max-w-[80%] {$isMobile
								? 'mt-[5vw] rounded-[2.5vw]'
								: 'mt-[2vw] rounded-[1vw]'}"
							alt="Flip Card Content"
						/>
					{/if}

					<div class={$isMobile ? 'mt-[3vw] text-[4.5vw]' : 'mt-[1vw] text-[1.35vw]'}>คำตอบคือ</div>
					<div
						class="rounded-full border-[0.15vw] border-white {$isMobile
							? `px-[5vw] py-[1vw] ${data.image ? 'mt-[2vw] text-[5.5vw]' : 'mt-[27vw] text-[7vw]'}`
