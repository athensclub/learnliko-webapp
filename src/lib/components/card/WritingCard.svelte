<script lang="ts">
	import type { WritingCard } from '$gql/generated/graphql';
	...
	export let data: WritingCard;
	export let onPassed: () => void;
	...
	let response = '';
	$: isFinished = response.length > 0;

	let results: { percentage: number; response: string; suggestion: string }[] = [
		{
			percentage: 1,
			response:
	...
	const evaluateResponse = () => {
		...
		// TODO: Implement gpt to evaluate user's response
...
<div class="relative min-h-[59.7vh] min-w-[83.3vw] rounded-[8.21vw] bg-white {clazz}">
	...
	{:else if !showResult}
		...
		<div in:fade={{ delay: 400 }} out:fade>
			...
			<textarea
				bind:value={response}
				class="my-[3.4vh] h-[33.1vh] w-full px-[5.4vw] text-[4.1vw] outline-none placeholder:font-bold placeholder:text-[#E0E0E0]"
	...
	{:else}
		...
		<div in:fade={{ delay: 400 }} out:fade>
			...
			>
				คำตอบของคุณ
				<br />
				<h3 class="text-[4.1vw]">ความถูกต้อง {currentResult.percentage}%</h3>
			</div>

			<!-- response -->
			<div class="flex h-[27.6vh] flex-col items-center justify-between border-b border-[#EAEAEA]">
				<div class="h-[18vh]">
				...
				<div class="mb-[2.3vh] flex h-[4.2vh] flex-row items-center">
					{#if results.length > 1}
						<button
							disabled={currentResultIndex <= 0}
							on:click={() => currentResultIndex--}
							class="rounded-[5.77vw] border border-black px-[3.3vw] text-[4.1vw] font-bold disabled:opacity-[0.2]"
							>←
						</button>
					{/if}
					<div class="w-[12vw] text-center text-[3.1vw] font-bold">
			...
			<!-- suggestion -->
			<div class="flex h-[15.2vh] flex-col border-b border-[#EAEAEA] px-[3.8vw] py-[1.1vh]">
				<h3 class="text-[4.1vw] font-bold">Suggest</h3>
				<p class="mt-[1vh] h-full overflow-y-scroll text-[3.6vw]">{currentResult.suggestion}</p>
			</div>

			<!-- bottom button -->
			<div class="mt-[1.9vh] flex h-full w-full justify-between px-[3.8vw]">
				<button
					disabled={isPassed}
					on:click={rewrite}
					class="rounded-[5.77vw] border border-black px-[6.2vw] py-[0.9vh] text-[4.1vw] font-bold disabled:opacity-[0.2]"
					>เขียนใหม่
				</button>
				<button
					disabled={!isPassed}
