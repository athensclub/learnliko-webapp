...
export const submitUserReply = async function (audioRecording: { data: Blob; url: string } | null) {
    ...
    if (audioRecording !== null) {
        ...
        history.set(get(history).map((v, i) => i === targetIndex ? { ...v, transcription: transcription } : v));
...
const computeRecap = async () => {
    ...
    for (let i = 0; i < get(history).length; i += 2) {
        ...
        promises.push(
            analyzeDialog(get(history)[i].transcription!, get(history)[i + 1].transcription!).then((recap) => {
                result[i / 2] = {
                    assistant: {
                        ...get(history)[i],
                        role: 'assistant',
                    ...
                    user: { ...get(history)[i + 1], role: 'user', transcription: get(history)[i + 1].transcription! },
