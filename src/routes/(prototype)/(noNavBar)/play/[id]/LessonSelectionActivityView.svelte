<script lang="ts">
	...
	import SelectionFlipCard from '$lib/components/SelectionFlipCard.svelte';
	...
	const updateIsMobile = () => {
		...
		if (displayed.length < targetCount) {
			while (items.length > 0 && displayed.length < targetCount) {
				let item = items[0];
				items = items.slice(1);
				displayed = [...displayed, item as SelectionCard];
			}
		} else {
	...
	const finishItem = (index: number) => {
		setTimeout(() => {
			if (items.length > 0) {
				let item = items[0];
				items = items.slice(1);
				displayed[index] = item as SelectionCard;
			} else {
...
>
	...
	{#each displayed as item, index (item)}
		<div
			...
			transition:fade
			class="absolute top-[45vw] {$isMobile ? 'h-[110vw] w-[75vw]' : 'h-[30vw] w-[22vw]'}"
		>
			<SelectionFlipCard
