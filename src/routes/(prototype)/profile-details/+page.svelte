...
<div class="flex flex-1 flex-col font-line-seed overflow-y-auto">
	<div
		...
		{#each progresses as progress}
			...
			>
				...
				<CircularProgressBar
					...
					value={progress.courseProgress[0].progress}
				>
					<div class="flex h-full flex-col items-center justify-center font-bold">
						...
						<div class={$isMobile ? 'text-[7vw]' : 'text-[2vw]'}>
							{progress.courseProgress[0].course.displayName}
						</div>
					</div>
				</CircularProgressBar>
				<button class="rounded-full text-[5vw] border border-black/[0.15] mt-[6vw] px-[10vw] py-[1.5vw] font-bold">Details</button>
			</div>
		{/each}
	</div>
