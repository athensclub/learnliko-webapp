<script lang="ts">
	...
	import { profileCurrentView, profileProgresses, selectedProfileProgress } from './+page.svelte';

	const selectProgress = (progress: SubjectProgress) => {
		$selectedProfileProgress = progress;
		$profileCurrentView = 'DETAILED';
	};
</script>
...
>
	...
	<div class="flex flex-col justify-center">
		<div class="text-[6.5vw] font-bold">{$userSession.authUser?.displayName ?? 'Guest'}</div>

		<div
			class="flex flex-row items-center gap-[3vw] font-bold {$isMobile
				? 'text-[6vw]'
				: 'ml-[4vw] text-[3vw]'}"
		>
			<div class="mt-[1vw] flex flex-row items-center">
				<div class="bg-gradient-to-r from-[#C698FF] to-[#6C80E8] bg-clip-text text-transparent">
					{Math.floor($userSession.accountData?.exp ?? 0)}
				</div>
				<svg
			...
			<div class="mt-[1vw] flex flex-row items-center">
				...
				>
					{Math.floor($userSession.accountData?.coin ?? 0)}
				</div>
	...
	{#each $profileProgresses as progress, index}
		{#each progress.courseProgress as courseProgress}
			<div
				...
				class="mt-[6vw] flex flex-col items-center rounded-[5vw] p-[8vw]"
			>
				<div class="text-[5.5vw] font-bold">{progress.subject.displayName}</div>
				<CircularProgressBar
					max={1}
					class="mt-[4vw] {$isMobile ? 'h-[42vw] w-[42vw]' : 'h-[12vw] w-[12vw]'}"
				...
				<button
					on:click={() => selectProgress(progress)}
					class="mt-[6vw] rounded-full border border-black/[0.15] px-[10vw] py-[1.5vw] text-[5vw] font-bold"
				>
					Details
				</button>
			</div>
